...
void finalizeMagicItem(struct MagicItem *magicItem)
{
    if (magicItem->trueDetails) {
        ...
        while (magicItem->trueDetails[i]) {
            free_or_die(magicItem->trueDetails[i]);
            ++i;
        }
        free_or_die(magicItem->trueDetails);
    }
    free_or_die(magicItem->trueDescription);
}
...
# define TYPE_COUNT 6
    ...
    int specialBeadTotal = roll("1d4+2", rnd);
...
static void generateSword(struct MagicItem *magicItem, struct rnd *rnd)
{
    ...
    if ( ! magicItem->trueDetails) {
        ...
        magicItem->trueDetails = reallocarray_or_die(magicItem->trueDetails,
                                                     detailCapacity,
                                                     sizeof(char *));
